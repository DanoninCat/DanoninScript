-- ============================================================
--  CAT EMPIRE | v5.15 | CODED FOR DANONIN
--  UI: Fluent Library
...
local function executeRollback()
    ...
    local ok, err = pcall(function()
        ...
        -- 1. STATS de cada fighter
        -- Snapshot tem Stats = {Attack=num, SPA=num, Ultimate=num}
        -- O servidor espera uma tabela de grades: {["S+"]=true, ["A"]=false...}
        -- Mapeamento de valor numérico para grade baseado nos valores reais do debug
        local function numToGrade(val)
            if not val then return nil end
            -- Valores vistos no debug: 2.45, 4.9, 7.45, 9.9, 12.45, 19.9
        ...
        if type(snap.Fighters) == "table" then
            for uid, fData in pairs(snap.Fighters) do
                ...
                -- Stats
                if type(fData.Stats) == "table" then
                    -- Usa Attack como grade principal (mais relevante)
                    local grade = numToGrade(fData.Stats.Attack)
                    if grade then
                        local gradeTable = {}
                        for _, g in ipairs(ALL_GRADES) do
                            gradeTable[g] = (g == grade)
                        end
                ...
                -- Locks do snapshot (StatsLocked = {} no debug = vazio, skip se vazio)
                if type(fData.StatsLocked) == "table" then
                    for statName, locked in pairs(fData.StatsLocked) do
                        if locked == true then
        ...
        -- 2. EQUIPA O MELHOR (restaura equip state)
        fire("General","Fighters","Equip_Best")
        task.wait(0.3)

        -- 3. QUEST — campo confirmado: snap.World_Quest = "Leaf Village"
        ...
        -- 4. TELEPORT para o mundo do snapshot
        -- Usa World_Quest se disponível, senão World
    ...
    if ok then
        Fluent:Notify({
            Title    = "Rollback",
            Content  = "Concluido! Rejoin em 2s...",
            Duration = 3,
        })
        task.wait(2)
...
local function simulateMinimize()
    -- Simula RightControl que o Fluent escuta via MinimizeKey
    pcall(function()
...
-- ============================================================
-- TAB: DEBUG — Remote Sniffer (Trial, Gacha e qualquer evento)
-- ============================================================
Tabs.Debug:AddSection("Remote Sniffer")

Tabs.Debug:AddParagraph({
    Title   = "Como usar",
    Content = "Ative o sniffer, execute uma acao no jogo (abrir Trial, Gacha, etc).\nOs remotes capturados aparecem como notificacoes e no output.",
})
...
Options.SnifferToggle:OnChanged(function(v)
    ...
    if v then
        ...
        local bridge = getBridge()
        if not bridge then
            snifferStatus:SetDesc("ERRO: Bridge nao encontrado!")
            return
        end

        -- Captura eventos que o SERVIDOR manda pro cliente
        ...
        if snifferConn then
            snifferConn:Disconnect()
            snifferConn = nil
        end
        snifferStatus:SetDesc("Inativo. " .. snifferCount .. " remotes capturados.")
        Fluent:Notify({ Title="Sniffer", Content="Desativado.", Duration=2 })
    end
end)

Tabs.Debug:AddButton({
    Title    = "Ver Capturados (output)",
    Callback = function()
        if #capturedRemotes == 0 then
            Fluent:Notify({ Title="Debug", Content="Nenhum remote capturado ainda.", Duration=3 })
            return
        end
        warn("====== REMOTES CAPTURADOS (" .. #capturedRemotes .. ") ======")
        for i, line in ipairs(capturedRemotes) do
            warn(i .. ": " .. line)
        end
        warn("====== FIM ======")
        Fluent:Notify({ Title="Debug", Content=#capturedRemotes .. " remotes no output.", Duration=3 })
    end,
})

Tabs.Debug:AddButton({
    Title    = "Limpar Capturados",
    Callback = function()
        capturedRemotes = {}
        snifferCount    = 0
        snifferStatus:SetDesc("Limpo.")
        Fluent:Notify({ Title="Debug", Content="Lista limpa.", Duration=2 })
    end,
})

Tabs.Debug:AddSection("Testar Remotes Manualmente")

-- Testa remotes de Trial/Gacha conhecidos
local TRIAL_TESTS = {
...
for _, t in ipairs(TRIAL_TESTS) do
    local tt = t
    Tabs.Debug:AddButton({
        Title    = "Testar: " .. tt.Label,
        Callback = function()
            fire(table.unpack(tt.Args))
            Fluent:Notify({
                Title   = "Teste",
                Content = tt.Label .. " enviado. Veja o output.",
                Duration = 3,
            })
            warn("[TESTE] " .. tt.Label .. " → " .. table.concat(
                (function()
                    local s = {}
                    for _, a in ipairs(tt.Args) do table.insert(s, tostring(a)) end
                    return s
                end)(), ", "
            ))
        end,
