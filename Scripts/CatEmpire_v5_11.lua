-- ============================================================
--  CAT EMPIRE | v5.11 | CODED FOR DANONIN
-- ============================================================
...
-- ============================================================
-- ROLLBACK: ESTRATÉGIA REAL
--
-- "Architect,Data,Receiver" é SERVER->CLIENT apenas.
-- O cliente NÃO pode mandar isso de volta — servidor ignora.
--
-- Solução: usar os remotes reais confirmados no debug (img 20)
-- para restaurar traits, stats, locks, quest e teleporte.
-- Remotes confirmados:
--   Traits:   General, Traits, Reroll, uid, traitTable
--   Stats:    General, StatsReroll, Reroll, uid, statsTable
--   Lock:     General, StatsReroll, Lock, uid, statName
...
local function executeRollback()
    ...
    Fluent:Notify({
        Title    = "Rollback",
        Content  = "Iniciando rollback do estado de " .. (rollbackTime or "?") .. "...",
        Duration = 3,
    })
    ...
    local ok, err = pcall(function()
        ...
        -- 1. TRAITS e STATS de cada fighter salvo no snapshot
        if type(snap.Fighters) == "table" then
            for uid, fData in pairs(snap.Fighters) do
                if type(fData) ~= "table" then continue end

                local snapTrait = fData.Trait

                -- Monta traitTable: só o trait do snapshot = true
                if snapTrait and snapTrait ~= "" then
                    local allTraits = {
                        "Strongest","Monarch","Clover III","Scholar","Clover II","Clover I",
                        "Fortune","Fortune II","Fortune III",
                        "Vigor","Vigor II","Vigor III",
                        "Swift","Swift II","Swift III",
                        "Lucky","Lucky II","Lucky III",
                    }
                    local traitTable = {}
                    for _, t in ipairs(allTraits) do
                        traitTable[t] = (t == snapTrait)
                    end
                    fire("General","Traits","Reroll", uid, traitTable)
                    task.wait(0.35)
                end

                -- Stats: monta tabela baseada nos valores numéricos do snapshot
                -- Confirmado no debug: Stats = {Attack, SPA, Ultimate}
                if type(fData.Stats) == "table" then
                    -- Heurística de grade baseada nos valores reais vistos
                    local function toGrade(val)
                        if not val or val == 0 then return "D" end
                        if val >= 15 then return "S+"
                        elseif val >= 12 then return "A+"
                        elseif val >= 9  then return "A"
                    ...
                    local allGrades = {"S+","A+","A","S-","C+","B+","D+","D","C-","D-"}
                    local snapGrade = toGrade(fData.Stats.Attack)
                    local statsTable = {}
                    for _, g in ipairs(allGrades) do
                        statsTable[g] = (g == snapGrade)
                    end

                    -- Só reroll se não tiver lock no snapshot
                    local hasLock = type(fData.StatsLocked) == "table" and fData.StatsLocked.Attack
                    if not hasLock then
                        fire("General","StatsReroll","Reroll", uid, statsTable)
                        task.wait(0.35)
                    end

                    -- Restaura locks que existiam no snapshot
    ...
    if ok then
        Fluent:Notify({
            Title    = "Rollback",
            Content  = "Restauracao concluida! Fazendo rejoin...",
            Duration = 3,
        })
        task.wait(1.5)
