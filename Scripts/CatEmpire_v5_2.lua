-- ============================================================
--  CAT EMPIRE | v5.2 | CODED FOR DANONIN
--  UI: Fluent Library
...
Options.AutoFarm:OnChanged(function(v)
    toggleLoop("AutoFarm", v, 0.35, function()
        -- getNearestEnemy agora retorna 3 valores: model, worldName, enemyID
        local enemy, world, enemyID = getNearestEnemy()
        if not enemy or not world or not enemyID then
            -- sem inimigo próximo: click básico
            fire("Fighters","Attack","Click")
            return
        end
        -- Attack: passa o model E o ID real do atributo "ID"
        fire("Fighters","Attack","Attack", world, enemy, enemyID)
        task.wait(0.05)
        -- Do_Damage recebe o ID do INIMIGO (não o UID do fighter)
