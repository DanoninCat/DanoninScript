-- ============================================================
--  CAT EMPIRE | v5.5 | CODED FOR DANONIN
--  UI: Fluent Library
...
local function getNearestEnemy()
    ...
    for _, worldFolder in ipairs(serverEnemies:GetChildren()) do
        -- Ignora Lobby (tem apenas Dummy com Health=inf)
        ...
        for _, enemy in ipairs(worldFolder:GetChildren()) do
            ...
            -- Ignora inimigos com Health infinito ou mortos
            local health = enemy:GetAttribute("Health")
            if not health or health <= 0 or health == math.huge then continue end

            -- Pega posição pelo PrimaryPart ou qualquer BasePart
            local enemyPos
...
-- Insere no CoreGui para ficar sempre visível
local ok2, screenGui = pcall(function()
    local sg = Instance.new("ScreenGui")
    sg.Name         = "CatEmpireMinGui"
