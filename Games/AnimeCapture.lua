-- ============================================================
--  DANONIN HUB | Anime Capture v3.0
...
local Window = Fluent:CreateWindow({
    ...
    Acrylic     = true,
    Theme       = Config.Theme,      -- "Darker"
    MinimizeKey = Enum.KeyCode.RightControl,

    -- Background image (suportado pelo Fluent custom)
    BackgroundImage             = Config.Background,
    BackgroundImageTransparency = Config.SetBackgroundImageTransparency,

    -- Ícone da janela (suportado pelo Fluent custom)
    Icon = Config.Icon,
})
...
-- ============================================================
-- BOTÃO FLUTUANTE — v3.0
-- Fix definitivo:
--   1. Frame.Active = true (consome input na layer)
--   2. TextButton.Active = true + Selectable = false
--   3. MouseButton1Down + InputBegan VAZIOS (consomem press/touch)
--   4. PPS.Enabled desativado ANTES de esconder a UI
--   5. Toggle via InputEnded (mais confiável no mobile)
-- ============================================================
local winVis   = true
local fluentSG = nil
...
local function doMin()
    ...
    -- Desativa ProximityPrompts ANTES de esconder a UI
    -- (evita NPC ativar no mesmo frame que a UI some)
    pcall(function() PPS.Enabled = newState end)
    task.wait()  -- 1 frame de delay
    if fluentSG then
...
Tabs.Info:AddParagraph({ Title="Script",  Content="Anime Capture v3.0" })
...
Fluent:Notify({
    Title    = "Danonin Hub",
    Content  = "Anime Capture v3.0 carregado!",
    Duration = 5,
})

warn("[AC] v3.0 carregado | Theme: "..Config.Theme.." | "..#RF:GetChildren().." remotes")
