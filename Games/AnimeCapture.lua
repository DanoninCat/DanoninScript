...
local function fire(name, ...)
    ...
    -- FIX: captura os args numa tabela ANTES da função anônima do pcall.
    -- "..." só é válido dentro da função que o declara — a função anônima
    -- do pcall não recebe varargs, então usar "..." nela diretamente
    -- causa "Cannot use '...' outside of a vararg function".
    local args = { ... }
    ...
    elseif r:IsA("RemoteFunction") then
        local ok, result = pcall(function() return r:InvokeServer(table.unpack(args)) end)
...
-- FIX: SentChatMessage (nome real confirmado no console) é um RemoteEvent
-- que o SERVIDOR dispara pro cliente. Sem OnClientEvent conectado, o
-- Roblox descarta o evento e gera o warning de "OnClientEvent".
do
    local chatEvent = RF:FindFirstChild("SentChatMessage")
