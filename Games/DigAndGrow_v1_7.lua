-- ============================================================
--  DIG & GROW | v1.7 | by Danonin
--  Remotes confirmados via debug hook:
--    CollectFruit:FireServer(string_UUID, 2)
--    MutationReceptor:InvokeServer(string_UUID, 1)
--    Shop:InvokeServer("Inventory")
--    ToolSignal:FireServer({Target, Position})  [auto dig]
--  Fix: botão minimizar com SetModalEnabled
-- ============================================================
...
-- ============================================================
-- HELPERS — PLANTAS
-- Debug confirmou: CollectFruit:FireServer(string_UUID, 2)
--                  MutationReceptor:InvokeServer(string_UUID, 1)
-- UUID é string (ex: "2c363ef5-8e79-40f2-af07-15fc30fc01bb")
-- ============================================================
    ...
    for _, obj in ipairs(plot:GetDescendants()) do
        ...
        if uid then
            local stage = obj:GetAttribute("Stage")
                       ...
                       or 0
            table.insert(plants, {
                UUID  = tostring(uid),   -- string UUID confirmado
...
-- COLLECT confirmado: CollectFruit:FireServer(string_UUID, 2)
-- Número 2 foi o visto no debug — provavelmente = quantidade ou tipo
local function collectAllReady()
    ...
    if #ready > 0 then
        for _, p in ipairs(ready) do
            pcall(function()
                R.Collect:FireServer(p.UUID, 2)
            end)
            count += 1
            task.wait(0.15)
        end
    else
        -- Fallback sem plot: dispara sem args (tenta coletar tudo)
        pcall(function() R.Collect:FireServer() end)
    end
    return count
end

-- MUTATE confirmado: MutationReceptor:InvokeServer(string_UUID, 1)
-- Número 1 visto no debug — provavelmente = nível de mutação mínimo
...
-- ============================================================
-- BOTÃO FLUTUANTE
-- Fix: SetModalEnabled bloqueia input do jogo durante clique/drag
-- ============================================================
local winVis = true
...
minHit.InputBegan:Connect(function(i)
    ...
    or i.UserInputType == Enum.UserInputType.Touch then
        ...
        UIS:SetModalEnabled(true)   -- bloqueia input do jogo
    end
end)
...
UIS.InputEnded:Connect(function(i)
    if i.UserInputType == Enum.UserInputType.MouseButton1
    or i.UserInputType == Enum.UserInputType.Touch then
        if drag then UIS:SetModalEnabled(false) end
        drag  = false
        moved = false
    end
end)
...
Tabs.Farm:AddDropdown("MutStage", {
    ...
    Default = "1",   -- debug mostrou [2]1 como padrão do jogo
})
...
Tabs.Shop:AddButton({
    ...
    Callback = function()
        if not R.Sell then
            Fluent:Notify({ Title="Erro", Content="Sell nao encontrado", Duration=4 })
            return
        end
        doSell()
...
-- TAB: INFO
-- ============================================================
Tabs.Info:AddSection("Dig & Grow v1.7")
...
Tabs.Info:AddParagraph({
    Title   = "v1.7 Fixes",
    Content = "- Collect: FireServer(UUID_string, 2) ✓\n- Mutate: InvokeServer(UUID_string, 1) ✓\n- Sell: InvokeServer('Inventory') ✓\n- Minimize: SetModalEnabled fix ✓",
})
...
Fluent:Notify({
    Title    = "Dig & Grow v1.7",
    Content  = "Carregado! Collect, Mutate e Sell corrigidos.",
    Duration = 5,
})
print("DIG & GROW v1.7 FULLY LOADED")
