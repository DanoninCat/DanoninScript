...
local function getPlants(plot)
    ...
    for _, obj in ipairs(plot:GetDescendants()) do
        ...
        -- UUID deve ser string não vazia e parecer um UUID/id válido
...
-- ============================================================
-- COLLECT PLOT — CollectFruit:FireServer(UUID, "1")
-- ============================================================
local function doCollect()
    ...
    if not plot then
        warn("[COLLECT] Plot nao encontrado — tentando sem UUID")
    ...
    if #plants == 0 then
        warn("[COLLECT] Nenhuma planta encontrada no plot")
        return 0
    end

    local count = 0
    for _, p in ipairs(plants) do
        print("[COLLECT] UUID:", p.UUID, "| Stage:", p.Stage, "| Harvestable:", p.Harvestable)
        pcall(function()
...
-- MouseButton1Down VAZIO — consome o clique antes de chegar no jogo
...
print("DIG & GROW v2.1 FULLY LOADED")
