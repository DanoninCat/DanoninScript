...
-- ============================================================
-- COLLECT
-- Tenta via GUI button click (mais confiável que remote direto)
-- ============================================================
local function findCollectButton()
    ...
    for _, obj in ipairs(pg:GetDescendants()) do
        if obj:IsA("TextButton") or obj:IsA("ImageButton") then
            ...
            local text = obj:IsA("TextButton") and obj.Text:upper() or ""
            if name:find("COLLECT") or text:find("COLLECT") then
                return obj
            end
        end
    end
...
local function doCollect()
    -- Tenta clicar botão COLLECT da GUI
    local btn = findCollectButton()
    if btn then
        pcall(function() btn:activate() end)
        return true
    end
    -- Fallback: dispara remote sem args
    if R.Collect then
...
local function getPlants(plot)
    ...
    for _, obj in ipairs(plot:GetDescendants()) do
        ...
        if uid then
            ...
            table.insert(plants, { UUID = tostring(uid), Stage = tonumber(stage) or 0 })
        end
    end
    return plants
end
...
-- ============================================================
-- BOTÃO FLUTUANTE — v1.9
-- Fix: quando UI está oculta, o botão fica transparente ao fundo
-- para não deixar área clicável que passa para o jogo.
-- Solução: usar um Frame invisível de tela cheia com absorção
-- de cliques APENAS quando a UI está fechada, para não abrir loja.
-- ============================================================
local winVis = true
...
-- ImageButton que SEMPRE absorve cliques (não passa para o jogo)
local minHit = Instance.new("ImageButton", minFrame)
minHit.Size                   = UDim2.fromScale(1, 1)
minHit.BackgroundTransparency = 1
minHit.Image                  = ""
minHit.ZIndex                 = 102
-- CRÍTICO: AutoButtonColor false evita highlight que denuncia clique ao jogo
...
local function doMin()
    ...
    if winVis then
        -- UI aberta: botão verde normal
        minFrame.BackgroundColor3 = Color3.fromRGB(20, 140, 60)
        stroke.Color              = Color3.fromRGB(60, 220, 100)
        minLbl.Text               = "D&G"
    else
        -- UI fechada: botão vermelho
...
Options.AutoCollect:OnChanged(function(v)
    Safety:Toggle("AutoCollect", v, 2, function() doCollect() end)
end)
...
Tabs.Farm:AddButton({
    ...
    Callback = function()
        doCollect()
        Fluent:Notify({ Title="Collect", Content="Executado!", Duration=2 })
    end,
})
...
-- Loop independente: coleta a cada 3s, vende a cada 15s
...
Options.AutoFullFarm:OnChanged(function(v)
    Safety:Toggle("AutoCollectFull", v, 3, function()
        doCollect()
    end)
    Safety:Toggle("AutoSellFull", v, 15, function()
        doSell()
    end)
end)
...
Options.AutoDig:OnChanged(function(v)
    Safety:Toggle("AutoDig", v, 0.5, function()
        ...
        if R.Tool then
            pcall(function()
                R.Tool:FireServer({ ["Target"]=floor, ["Position"]=hrp and hrp.Position or Vector3.new(0,0,0) })
            end)
        end
    end)
end)
...
Tabs.Garden:AddButton({
    ...
    Callback = function()
        ...
        Fluent:Notify({ Title="Plot", Content=#plants.." planta(s) encontrada(s)", Duration=3 })
    end,
})
...
print("DIG & GROW v1.9 FULLY LOADED")
