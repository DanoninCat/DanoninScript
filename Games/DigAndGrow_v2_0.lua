-- ============================================================
--  DIG & GROW | v2.0 | by Danonin
--  Debug confirmou:
--    CollectFruit:FireServer(string UUID, string "1")
--    MutationReceptor:InvokeServer(string UUID, string "1")
--  Fix minimize: TextButton com Modal=true só enquanto fechado
-- ============================================================
...
local function getPlants(plot)
    ...
    for _, obj in ipairs(plot:GetDescendants()) do
        local uid = obj:GetAttribute("UUID") or obj:GetAttribute("Id")
                 or obj:GetAttribute("ID")   or obj:GetAttribute("PlantId")
                 or obj:GetAttribute("FruitId")
        if uid then
            local stage = obj:GetAttribute("Stage") or obj:GetAttribute("GrowthStage")
                       or obj:GetAttribute("Growth") or 0
            table.insert(plants, {
                UUID  = tostring(uid),
...
-- ============================================================
-- COLLECT — confirmado: FireServer(string UUID, string "1")
-- ============================================================
local function doCollect()
    if not R.Collect then return 0 end
    ...
    if #plants > 0 then
        for _, p in ipairs(plants) do
            -- Ambos args são STRING conforme debug
            pcall(function()
                R.Collect:FireServer(p.UUID, "1")
            end)
            count += 1
            task.wait(0.2)
        end
    else
        -- Fallback sem plot: tenta sem args
...
-- ============================================================
-- MUTATE — confirmado: InvokeServer(string UUID, string stage)
-- ============================================================
local function doMutate(stageNum)
    ...
    local plants = getPlants(plot)
    local count  = 0
    local stageStr = tostring(stageNum)  -- arg é STRING
...
-- ============================================================
-- SELL — confirmado: InvokeServer("Inventory")
-- ============================================================
...
-- Fix definitivo minimize:
--   - TextButton com Modal=true SEMPRE (bloqueia jogo atrás)
--   - Modal só bloqueia a camada do jogo, não outros ScreenGuis
--   - Drag via RenderStepped para não depender de InputChanged
-- ============================================================
local winVis = true
...
-- TextButton com Modal=true: bloqueia qualquer input do jogo atrás
-- mesmo quando a UI do Fluent está oculta
local minHit = Instance.new("TextButton", minFrame)
minHit.Size                   = UDim2.fromScale(1, 1)
minHit.BackgroundTransparency = 1
minHit.Text                   = ""
minHit.Modal                  = true   -- ← bloqueia jogo atrás SEMPRE
...
-- Drag simples sem SetModalEnabled
...
-- Collect a cada 3s + Sell a cada 15s, loops independentes
...
Options.AutoFullFarm:OnChanged(function(v)
    Safety:Toggle("AFF_Collect", v, 3,  function() doCollect() end)
...
Options.AutoDig:OnChanged(function(v)
    Safety:Toggle("AutoDig", v, 0.5, function()
        ...
        if not floor or not R.Tool then return end
...
Tabs.Garden:AddButton({
    ...
    Callback = function()
        ...
        if not R.Tool then Fluent:Notify({ Title="Garden", Content="Tool remote nao encontrado!", Duration=3 }) return end
        local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
        pcall(function()
            R.Tool:FireServer({ Target = floor, Position = hrp and hrp.Position or Vector3.zero })
        end)
...
Tabs.Garden:AddButton({
    ...
    Callback = function()
        ...
        local plants = getPlants(plot)
        plotPara:SetDesc("Plot: "..plot.Name.."\nPlantas detectadas: "..#plants)
        Fluent:Notify({ Title="Plot", Content=#plants.." planta(s)", Duration=3 })
    end,
})
...
Fluent:Notify({
    Title    = "Dig & Grow v2.0",
    Content  = "Carregado! Collect e Mutate corrigidos.",
    Duration = 4,
})
print("DIG & GROW v2.0 FULLY LOADED")
