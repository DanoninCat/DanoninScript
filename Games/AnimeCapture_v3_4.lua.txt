-- ============================================================
--  DANONIN HUB | Anime Capture v3.4
...
-- FIX: valida que Fluent é realmente utilizável, não só "não-nil"
-- Se Libs/Fluent.lua estiver incompleto/corrompido, isso pega ANTES de
-- explodir depois em Fluent:CreateWindow() com erro sem contexto
if type(Fluent) ~= "table" or type(Fluent.CreateWindow) ~= "function" then
    warn("[AC] Fluent carregado mas incompleto/incompativel (falta CreateWindow)")
    warn("[AC] Verifique se Libs/Fluent.lua no GitHub esta completo e correto")
    return
end
...
if type(SaveManager) ~= "table" or type(SaveManager.SetLibrary) ~= "function" then
    warn("[AC] SaveManager carregado mas incompleto/incompativel (falta SetLibrary)")
    return
end
...
if type(InterfaceManager) ~= "table" or type(InterfaceManager.SetLibrary) ~= "function" then
    warn("[AC] InterfaceManager carregado mas incompleto/incompativel (falta SetLibrary)")
    return
end

warn("[AC] Fluent/SaveManager/InterfaceManager validados OK")
...
-- FIX: fire() agora identifica o tipo real do remote em vez de assumir
-- FireServer sempre. Erros não ficam mais escondidos no pcall — aparecem
-- no output como warn para facilitar diagnóstico.
local function fire(name, ...)
    local r = RF:FindFirstChild(name)
    if not r then
        warn("[AC] Remote nao encontrado: " .. name)
        return
    end

    if r:IsA("RemoteEvent") then
        local ok, err = pcall(function() r:FireServer(...) end)
        if not ok then warn("[AC] FireServer falhou [" .. name .. "]: " .. tostring(err)) end

    elseif r:IsA("RemoteFunction") then
        local ok, result = pcall(function() return r:InvokeServer(...) end)
        if not ok then warn("[AC] InvokeServer falhou [" .. name .. "]: " .. tostring(result)) end
        return result

    else
        warn("[AC] Tipo invalido [" .. name .. "]: " .. r.ClassName)
    end
end

-- FIX: SendChatMessage é um RemoteEvent que o SERVIDOR dispara pro cliente
-- (chat do jogo). Sem OnClientEvent conectado, o Roblox descarta o evento
-- e gera o warning "did you forget to implement OnClientEvent?".
-- Conectar um listener vazio silencia isso sem afetar nada do hub.
do
    local chatEvent = RF:FindFirstChild("SendChatMessage")
    if chatEvent and chatEvent:IsA("RemoteEvent") then
        pcall(function()
            chatEvent.OnClientEvent:Connect(function(...) end)
        end)
    end
end
...
Tabs.Info:AddParagraph({ Title="Script",  Content="Anime Capture v3.4" })
...
Fluent:Notify({
    Title    = "Danonin Hub",
    Content  = "Anime Capture v3.4 carregado!",
    Duration = 5,
})

warn("[AC] v3.4 carregado | Theme: "..Config.Theme.." | "..#RF:GetChildren().." remotes | fluentSG: "..(fluentSG and fluentSG.Name or "nil"))
