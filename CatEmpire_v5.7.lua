-- ============================================================
--  CAT EMPIRE | v5.7 | CODED FOR DANONIN
--  UI: Fluent Library
...
local function getCurrentWorld()
    -- Prioridade 1: playerData interceptado via OnClientEvent
    if playerData and playerData.World and playerData.World ~= "" then
        return playerData.World
    end
    -- Prioridade 2: procura pela pasta do mundo que contém o personagem
        ...
        for _, enemy in ipairs(worldFolder:GetChildren()) do
            ...
            if enemy.PrimaryPart then
                ...
                for _, p in ipairs(enemy:GetDescendants()) do
                    if p:IsA("BasePart") then enemyPos = p.Position break end
                end
            end
            if enemyPos then
                local d = (hrp.Position - enemyPos).Magnitude
...
local function getNearestEnemy()
    ...
    -- Só ataca inimigos do mundo onde o personagem está
    ...
    for _, enemy in ipairs(worldFolder:GetChildren()) do
        ...
        if enemy.PrimaryPart then
            ...
            for _, part in ipairs(enemy:GetDescendants()) do
                if part:IsA("BasePart") then enemyPos = part.Position break end
            end
        end
...
-- Encontra o ScreenGui do Fluent após um frame
task.defer(function()
    for _, sg in ipairs(game:GetService("CoreGui"):GetChildren()) do
        if sg:IsA("ScreenGui") and sg.Name:find("Fluent") then
            fluentRoot = sg break
        end
    end
...
minFloatBtn.MouseButton1Click:Connect(function()
    ...
    -- Tenta API do Fluent primeiro
    pcall(function() Window:SetMinimized(not windowVisible) end)
    -- Fallback: esconde/mostra o ScreenGui inteiro
    if fluentRoot then
...
Options.AutoFarm:OnChanged(function(v)
    toggleLoop("AutoFarm", v, 0.35, function()
        ...
        -- Teleporta perto do inimigo para passar checagem de posição do servidor
        local char = player.Character
        local hrp  = char and char:FindFirstChild("HumanoidRootPart")
        if hrp then
            local enemyPos
...
-- NPC alvo
local targetNPCName = "Qualquer"
local npcListCache  = { "Qualquer" }
...
getNearestEnemy = function()
    if targetNPCName == "Qualquer" then
...
local function refreshNPCList()
    ...
    npcDropdown:SetValue("Qualquer")
    targetNPCName = "Qualquer"
    Fluent:Notify({
        Title   = "NPCs",
        Content = (#names - 1) .. " NPC(s) em " .. (currentWorld or "?"),
        Duration = 3,
    })
end
